...
export const GRID_WRAPPER_ID = 'grid-outer-wrapper';
...
export function useDrag(props: {
  ...
  const onDragEnd = (e: MouseEvent) => {
    ...
    const gridBox = document.getElementById(GRID_WRAPPER_ID)?.getBoundingClientRect()!;
    const elBox = elementRef.current?.getBoundingClientRect()!;
    
    const targetNodePos = {
      x: Math.round((elBox.x - gridBox.left) / nodeSize),
      y: Math.round((elBox.y - gridBox.top) / nodeSize),
    };
  ...
  function snapToGrid(pos: Pos) {
    if (elementRef.current && id.current) {
